...
There is now a python visualisation script, which uses `matplotlib` and `pandas` to interpret collected data from the CSV Logger. This can be activated by installing the dependencies on `requirements.txt` and building with the `BUILD_PLOTTING_TOOLS` option on. You can do this by entering:
